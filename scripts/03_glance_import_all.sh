...
LOG_FILE="$LOG_DIR/03_glance_import_all.log"
...
[[ -x "$SCRIPT_DIR/02_glance_import_one.sh" ]] || die "script not executable: $SCRIPT_DIR/02_glance_import_one.sh"
...
for version in "${versions[@]}"; do
  log "import start version=$version"
  "$SCRIPT_DIR/02_glance_import_one.sh" "$version"
  log "import done version=$version"
done
