...
* ตรวจ repo เดิมก่อน
* สลับไป OLS อย่างปลอดภัย
* ถ้า OLS พัง ให้ rollback กลับ official
* อัปเดตระบบ
* reboot ถ้าจำเป็น
...
* พังตอน OLS injection
* พังตอน apt update หลังสลับ OLS
* พังตอน reboot
* พังตอน final clean
...
* backup repo files
* OLS mirror injection
* OLS validation
* rollback ถ้า OLS fail
* ไปต่อด้วย official ถ้า rollback สำเร็จ

## Vault Fallback Policy (new)

ลำดับการเลือก repo: **official → OLS → vault → official-fallback → failed**

* vault ถูกเรียกเมื่อ:

  * official degraded ที่ ANY phase (ตรวจพบระหว่าง configure)
  * OLS fail และ rollback official ยังใช้ไม่ได้
...
ถ้าทุก repo mode ล้มเหลว (official + OLS + vault):
...
|---|---|---|
| `official` | `ols_skip` | OLS ถูก disable |
| `ols` | `ols_ok` | OLS ใช้ได้ปกติ |
| `vault` | `ols_failed` | OLS fail → vault |
| `vault` | `official_degraded` | official degraded → vault |
| `official-fallback` | `ols_failed` | OLS fail → rollback official สำเร็จ |
...
เราไม่ควรไปลอง OLS ต่อ เพราะจะทำให้สืบ root cause ยากขึ้น
...
phase นี้คือเปลี่ยนจาก official ไป OLS แบบมี control
...
* OLS inject สำเร็จไหม
* OLS validation ผ่านไหม
...
* phase ชัดจาก preflight → baseline → OLS → fallback → update → reboot → access policy → system policy → clean → validate → shutdown

## ด้าน failure handling

* ถ้า OLS พัง ให้ rollback official แล้วไปต่อได้
...
* phase configure มีลำดับชัดและมี OLS failover
